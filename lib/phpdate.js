...
function ResultCacheRing() {
    this.cache = new Array();   // fastest variant
...
function gmdate( format, timestamp ) {
    ...
    if (_cacheResults && (output = _resultCacheGmt.get(format, dt))) return output;

    var output = _phpdate(format, new GmtDate(dt));
    if (_cacheResults) _resultCacheGmt.set(format, dt, output);
