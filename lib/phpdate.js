/**
 * php.date() work-alike, supports all conversions exactly like php
 *
...
module.exports.timezoneName = null;

var fs = require('fs');
try { module.exports.timezoneName = fs.readFileSync('/etc/timezone').toString().trim(); }
catch (err) { }

function configure( config ) {
config = config || {};

var _cacheTimezone = getConfig(config, 'cacheTimezone', true);
var _cacheCurrentDate = getConfig(config, 'cacheCurrentDate', false);
// result caching helps when at least 2 out of 5 timestamps are duplicates
var _cacheResults = getConfig(config, 'cacheResults', true);
...
var _emptyCache = { get: function() { return undefined }, set: function() {} };
function ResultCacheRing() {
    //this.cache = new Array(); // fast
    //this.cache = new Array(4); // slow
    //this.cache = [,,,,,,,,,]; // slow
    this.cache = [0,0,0,0]; // fast
    this.index = 0;
}
...
ResultCacheRing.prototype.get = function( format, dt ) {
    ...
    for (i=0; i<len; i++) {
        var item = cache[i];
        if (item && item.ts === tm && item.fmt === format && item.gmt === dt.isGmt) {
            return item.res;
        }
    }
    return undefined;
...
function phpdate( format, dt, options ) {
    ...
    var output;
    var cache = _cacheResults ? _resultCache : _emptyCache;
    ...
    if (options) {
        ...
        cache = _cacheResults ? _resultCache : _emptyCache;
    }
    ...
    if ((output = cache.get(format, dt)) !== undefined) return output;

    // use a custom function to typeset the date, avoid many small calls
    cache.set(format, dt, (output = _phpdate(format, dt)));
...
function _clearCurrentDate() { _currentDate = null; }
function _timestampToDt( timestamp ) {
    if (timestamp !== undefined) {
        ...
        setTimeout(_clearCurrentDate, 1);
...
var actions = {
    ...
    s: '( fmt.pad2(now.getSeconds()) )',
        // javascript Date does not have microsecond precision, format millis and pad
    u: '( fmt.pad3(now.getTime() % 1000) + "000" )',
...
//    if (module.exports.timezoneName) {
//        // TODO: qdate.getTimezoneAbbrev(module.exports.timezoneName);
//        var tzCode = now.toString().slice(35, 38);
//        var tzName = module.exports.timezoneName;
//    }
//    else {
        var tzMap = {
...
//          '0--60':   ['EGT', 'EGST', 'Eastern Greenland'],        // EG Time and EG Summer Time, also 'Europe/London' (GMT/BST,BDST)
        ...
        // TODO: use qdate to probe and manage timezone offsets
...
// assemble the exported functionality
phpdate.date = phpdate;
phpdate.gmdate = gmdate;
phpdate._phpdate = _phpdate;
phpdate.timezoneName = module.exports.timezoneName;
phpdate.configure = configure;

return phpdate;
}

function getConfig( configs, name, defaultValue ) {
    return configs[name] !== undefined ? configs[name] : defaultValue;
}
